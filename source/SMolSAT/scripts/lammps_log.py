...
class lammps_log:
  ...
  def get(self,*keys):
    ...
    if len(keys) == 1: return np.array(vecs[0])
    else: return np.array(vecs)
    ...
    while not last:
      ...
      if self.style == 1:
        ...
        for section in sections:
          ...
          words = word1 + word2
          self.data.append([float(word) for word in words])
      ...
      #print(int(self.data[len(self.data)-1][0]),sys.stdout.flush())
